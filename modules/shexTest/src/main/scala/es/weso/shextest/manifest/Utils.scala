...
object Utils {
  ...
  val negativeSyntaxBase = new java.net.URI(
    "https://raw.githubusercontent.com/shexSpec/shexTest/master/negativeSyntax/manifest"
  )
  val negativeStructureBase = new java.net.URI(
    "https://raw.githubusercontent.com/shexSpec/shexTest/master/negativeStructure/manifest"
  )
  val schemasBase =
    new java.net.URI("https://raw.githubusercontent.com/shexSpec/shexTest/master/schemas/manifest")
  val validationBase = new java.net.URI(
          ...
          for {
            ...
            ok <-
              if (v.traits contains sht_Greedy) {
                ...
                val shapeMap =
                  FixedShapeMap(Map(focus -> Map(lbl -> Info())), dataPrefixMap, schema.prefixMap)
                for {
                  ...
                  _ <- testInfo(
                    s"resultShapeMap: ${resultShapeMap.showShapeMap(verbose.asBoolean)}",
                    verbose
                  )
                  ok <-
                    if (resultShapeMap.getConformantShapes(focus) contains lbl) {
                      if (shouldValidate)
                        ...
                        result(
                          name,
                          false,
                          ConformsButShoudnt(focus, lbl, dataStr, schemaStr, resultShapeMap)
                        )
                    } else {
                      if (!shouldValidate) result(name, true, DoesntValidateAsExpected)
                        ...
                        result(
                          name,
                          false,
                          DoesntConformButShould(focus, lbl, dataStr, schemaStr, resultShapeMap)
                        )
                    }
                } yield ok
              }
