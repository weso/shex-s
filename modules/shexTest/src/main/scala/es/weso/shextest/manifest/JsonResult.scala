...
case class JsonResult(rmap: Map[RDFNode, List[ShapeResult]]) {
  def compare(resultMap: ResultShapeMap): Boolean = {
    ...
    rmap
      .map { case (node, ls) =>
        ls.map { case sr =>
          check(node, sr.shapeMapLabel, sr.value)
        }
      }
      .flatten
      .forall(identity)
...
object JsonResult {
  ...
  implicit val rdfNodeDecoder: Decoder[RDFNode] = Decoder.instance { c =>
    c.as[String]
      .flatMap { s =>
        RDFNode.fromString(s).fold(s => Left(DecodingFailure(s, Nil)), node => Right(node))
      }
  }

  implicit val rdfnodeKeyDecoder: KeyDecoder[RDFNode] = new KeyDecoder[RDFNode] {
    override def apply(key: String): Option[RDFNode] =
      RDFNode.fromString(key).fold(_ => None, Some(_))
  }
