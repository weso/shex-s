...
class RDF2Manifest extends RDFParser {
  ...
  def local[A](
      fn: ManifestContext => ManifestContext,
      parser: ManifestParser[A]
  ): ManifestParser[A] =
  ...
  private def compoundResult: RDFParser[ResultExpected] =
    for {
      ...
      v <- maybeType match {
        ...
        case Some(other) =>
          parseFail[ResultExpected](s"Unsupported type of compound result: $other")
      }
    } yield v
          ...
          result <- {
            def parseIri(iri: IRI): ManifestParser[(IRI, Option[ShExManifest])] =
              if (ctx.visited contains iri)
                liftParser(
                  parseFail(
                    s"Iri $iri alread visited. Visited=${ctx.visited.map(_.toString).mkString(",")}"
                  )
                )
              else
    ...
    pair <- node match {
      case iri: IRI =>
        if (ctx.derefIncludes) {
          ...
          val r: ManifestParser[(IRI, Option[ShExManifest])] =
            parseShExManifest(iriResolved, ctx, config)
          r
...
object RDF2Manifest {
  ...
  private def mkShExManifest(cfg: Config, ctx: ManifestContext): IO[List[ShExManifest]] = {
    val mm: IO[Either[Throwable, List[ShExManifest]]] =
      (new RDF2Manifest).rdf2Manifest.run(ctx).value.run(cfg)
    mm.flatMap(
      _.fold(
