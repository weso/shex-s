...
trait RunManifest {
  ...
  def runManifest(
    ...
    for {
      mf <- RDF2Manifest.read(
        Paths.get(fileName),
        "TURTLE",
        Some(s"$parentFolderURI/$folder/"),
        true
      )
  ...
  private def runWithTimeout(
      name: String,
      action: IO[Option[Result]],
      timeout: FiniteDuration
  ): IO[Option[Result]] = {
    ...
    for {
      rs1 <- m.includes
        .map { case (includeNode, manifest) =>
          val folder =
            Try(Paths.get(includeNode.getLexicalForm).getParent.toString).getOrElse("")
          runManifest(
...
object ValidateManifest extends RunManifest {
  ...
  def parseManifest(
    ...
    testInfo(
      s"Parse manifest: name: ${name}, folder: $folderName, parentFolder: ${testsFolder}",
      verbose
    ) *>
      runManifest(
        name,
  ...
  def representationTest(repTest: RepresentationTest, folderURI: URI): IO[Option[Result]] = {
    ...
    val resolvedShEx =
      mkLocal(repTest.shex, schemasBase, folderURI) // IRI(shexFolderURI).resolve(r.shex).uri
