...
object Reason {
  ...
  final case class JsonResultsDifferent(obtained: ResultShapeMap, expected: JsonResult)
      extends Reason
  final case class ConformantMatch(focus: RDFNode, lbl: ShapeMapLabel, result: ResultShapeMap)
      extends Reason
  final case class ConformsButShoudnt(
      focus: RDFNode,
      lbl: ShapeMapLabel,
      dataStr: String,
      schemaStr: String,
  ...
  final case class DoesntConformButShould(
      focus: RDFNode,
      lbl: ShapeMapLabel,
      dataStr: String,
      schemaStr: String,
  ...
  final case class JsonsDifferent(schema: Schema, jsonGenerated: Json, expected: Json)
      extends Reason
  ...
  final case class SchemaParsedWithNegativeStructure(schema: Schema, schemaStr: String)
      extends Reason
  ...
  final case class Timeout(name: String, duration: FiniteDuration) extends Reason {
    override def toString =
      s"""Timeout on action ${name}. Time took more than ${duration.toSeconds} seconds"""
  }
}
