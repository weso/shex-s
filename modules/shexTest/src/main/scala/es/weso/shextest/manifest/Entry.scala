          ...
          r <-
            if (CompareSchemas.compareSchemas(schema, expectedSchema)) {
              parse(jsonStr) match {
                case Left(err) =>
                  IO.pure(
                    FailedResult(id, msg = Some(s"Schemas are equal but error parsing Json $jsonStr"))
                  )
                case Right(json) => {
      ...
      action = {
        ...
        action match {
          case focusAction: FocusAction =>
            validateFocusAction(focusAction, base, this, true, name, uri, None, verbose).map(maybeR =>
              maybeR match {
                case None => FailedResult(id, Some("No result"), None, None)
                case Some(res) =>
                  if (res.isOk == shouldPass) {
          ...
          case mr: MapResultAction =>
            validateMapResult(mr, base, this, name, uri, verbose).map(maybeR =>
              maybeR match {
                case None => FailedResult(id, Some("No result"), None, None)
                case Some(res) =>
                  if (res.isOk) {
...
) extends ValidOrFailureTest(node, status, name, traits, comment, action, maybeResult, sht_ValidationTest, true) {}
...
case class ValidationFailure(
    ...
    override val action: Action,
    override val maybeResult: Option[IRI]
) extends ValidOrFailureTest(node, status, name, traits, comment, action, maybeResult, sht_Validate, false) {}

case class NegativeSyntax(override val node: RDFNode, override val status: Status, override val name: String, shex: IRI)
    extends Entry {
