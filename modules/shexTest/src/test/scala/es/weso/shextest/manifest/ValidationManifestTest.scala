...
//  val shexFolder = conf.getString("shexLocalFolder")
  ...
  test("run all") {
    ...
    val cmp: IO[TestResults] = for {
      ...
      _ <- IO.println(
        s"Failed: ${results.failed.length}\n${results.failed.map(_.show).mkString("\n")}"
      )
    } yield results

    cmp.map(vs => assertEquals(vs.failed, Vector[FailedResult]()))
  }
  ...
  test("single") {
    val cmp: IO[TestResult] = for {
      ...
      result <- testSuite.runSingle(
        TestId("vitals-RESTRICTS-fail_sit-ReclinedVital"),
        TestConfig(timeoutByTest, true)
      )
    } yield result
    cmp.map(res => assertEquals(res.passed, true))
  }
