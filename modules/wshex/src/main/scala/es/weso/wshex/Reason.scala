...
case class NoMatch(bag: Bag[(PropertyId,ShapeLabel)], rbe: Rbe[(PropertyId, ShapeLabel)], errors: NonEmptyList[RbeError]) extends Reason(errCode = Reason.noMatch)
...
case class CardinalityError(p: PropertyId, count: Int, min: Int, max: IntOrUnbounded) extends Reason(errCode = Reason.cardinalityError)
case class WaitingForFailed(es: Set[(Value, PropertyId, ShapeLabel)]) extends Reason(errCode = Reason.waitingForFailed)
case class MatchNot(bag: Bag[(PropertyId,ShapeLabel)], rbe: Rbe[(PropertyId, ShapeLabel)]) extends Reason(errCode = Reason.matchNot)
...
case class NotAllowedNotInExtra(notAllowed: List[(PropertyId, Int)]) extends Reason(errCode = Reason.notAllowedNotInExtra)
