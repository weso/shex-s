  ...
  def checkLocal(
    ...
    val result: Either[ReasonCode,Set[ShapeLabel]] = this match {
      ...
      case so@ShapeOr(_,ls) => {
        ...
        def cmb(v1: Either[ReasonCode,Set[ShapeLabel]], current: Either[ReasonCode,Set[ShapeLabel]]): Either[ReasonCode,Set[ShapeLabel]] =
          v1 orElse current
        val rs = vs.foldRight(zero)(cmb)
        rs
      }
...
sealed abstract class NodeConstraint extends ShapeExpr {
  ...
  def matchLocalCoded(value: Value): Either[ReasonCode, Unit] = matchLocal(value).leftMap(r => r.errCode)
}
