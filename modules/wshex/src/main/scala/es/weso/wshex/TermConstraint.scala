...
case class LabelConstraint(lang: Lang, strConstraint: StringConstraint) extends TermConstraint {
  override def matchTerm(ed: TermedDocument): Either[String, Unit] = {
    val labelsMap = ed.getLabels()
    val value = labelsMap.get(lang.lang)
    if (value == null) s"LabelConstraintFailed: Not found value for lang $lang for term $ed".asLeft
    else strConstraint.matchMonolingualTextValue(value)
  }
}
case class DescriptionConstraint(lang: Lang, strConstraint: StringConstraint)
...
case class AliasConstraint(lang: Lang, strConstraint: StringConstraint) extends TermConstraint {
  override def matchTerm(ed: TermedDocument): Either[String, Unit] = {
    ...
    if (values == null)
      ...
      values.asScala.toList
        .map(v => strConstraint.matchMonolingualTextValue(v))
        .sequence
        .map(_ => ())
    }
  }
...
/* Example of constraints
 ...
 rdfs:label @en AND /Tim/ // label in English matches regex pattern Tim
 skos:prefLabel @es   // label in Spanish
 */
