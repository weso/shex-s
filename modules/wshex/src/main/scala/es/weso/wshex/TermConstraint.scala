...
object TermConstraint {
  ...
  private def matchLangsMap(
    ...
    val zero: Either[MatchingError, Map[String, MonolingualTextValue]] =
      Map[String, MonolingualTextValue]().asRight
    def cmb(
  ...
  private def matchLangsMapLs(
    ...
    val zero: Either[MatchingError, Map[String, List[MonolingualTextValue]]] =
      Map[String, List[MonolingualTextValue]]().asRight
    def cmb(
        pair: (Lang, Option[StringConstraint]),
  ...
  private def addMapValues(
      ...
      updateFn: (EntityDoc, String, MonolingualTextValue) => EntityDoc
  ): EntityDoc = {
  ...
  private def addMapValuesLs(
      ...
      updateFn: (EntityDoc, String, List[MonolingualTextValue]) => EntityDoc
  ): EntityDoc = {
  ...
  private def optMatchConstraints(
    ...
    maybesc match {
      ...
      case Some(sc) =>
        values.foldRight(List[MonolingualTextValue]().asRight[MatchingError]) { case (v, current) =>
          for {
  ...
  case class LabelAny(strConstraint: Option[StringConstraint]) extends TermConstraint {
  ...
  case class DescriptionAny(strConstraint: Option[StringConstraint]) extends TermConstraint {
    override def matchTerm(
        ed: EntityDoc,
        current: EntityDoc
    ): Either[MatchingError, EntityDoc] = {
      ...
      if (descsMap.isEmpty) DescrAnyNoDescr(ed).asLeft
        ...
        descsMap.toList.foldLeft(current.asRight[MatchingError]) { case (c, pair) =>
          val (lang, value) = pair
          for {
