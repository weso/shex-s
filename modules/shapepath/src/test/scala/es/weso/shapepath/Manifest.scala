...
case class ManifestEntry(
  ...
  def processEntryPositive(
    ...
    readContents2(manifestPath + from, manifestPath + expect + ".json").use { pair =>
      ...
      for {
        ...
        (es, v) = pair
        _ <-
          if (!es.isEmpty) IO.println(s"Processing errors: ${es.map(_.toString + "\n").mkString}")
          else IO.pure(())
      } yield
  ...
  def processEntryNegative(id: TestId, shapePath: String, msgExpected: String): IO[TestResult] =
    ShapePath
      .fromString(shapePath)
      .fold(
        err =>
          IO.pure(PassedResult(id, msg = Some(s"Failed to parse as expected with msg: ${err}"))),
        v =>
          IO.pure(
            FailedResult(
              id,
              msg = Some(s"Parsed as $v but it was expected to fail\nshapePath: $shapePath")
            )
          )
      )
...
case class ManifestErrorParsingJson(failure: DecodingFailure, line: String, json: String)
    extends RuntimeException(
      s"Error obtaining manifest from json: ${failure.message}\nLine: ${line}\nJson:\n ${json}"
    )
case class Manifest(
    description: String,
      ...
      case _ =>
        ...
        val broken = parse(
          cursor
            ...
            .get
            .toString
            .replace(s""""$target"""", ">something that will break it!")
        )
...
//val errorMessageContainingLineNumber = historyToLineNumber(hcursor.downField("Top"), history.reverse)
  ...
  def fromJson(json: Json): IO[Manifest] = json
    .as[Manifest]
    .fold(
      f => {
  ...
  def fromString(str: String): IO[Manifest] = for {
    json <- IO.fromEither(
      parse(str).leftMap(df => new RuntimeException(s"JSON parsing error: ${df.message}"))
    )
