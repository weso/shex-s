...
/*      case _ if isDefined(ctx.relativePathExpr()) =>
  ...
  override def visitStepExpr(ctx: StepExprContext): Builder[Step] = ctx match {
    ...
    case _ if isDefined(ctx.postfixExpr()) =>
      err(s"visitStepExpr: TODO postfixExpr: $ctx / ${ctx.getClass.getName}")
  ...
  override def visitStringLiteral(ctx: StringLiteralContext): Builder[String] = ctx match {
    case _ if isDefined(ctx.STRING_LITERAL1()) =>
  ...
  override def visitTripleExprLabel(
      ctx: TripleExprLabelContext
  ): Builder[(ShapeLabel, Option[Int])] = for {
    maybeInt <-
