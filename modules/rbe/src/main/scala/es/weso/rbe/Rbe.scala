...
/** This trait defines Single Occurrence Regular Bag Expressions (Rbe)
  ...
  * ==Further info==
  *
  * The algorithm to check that a Rbe contains a bag is PTIME The algorithm has been described in [1] and is based on
  * intervals
  *
  * [1] Complexity and Expressiveness of ShEx for RDF, S. Staworko, I. Boneva, J. Labra, S. Hym, E. Prud'hommeaux, H.
  * Solbrig
  */
sealed trait Rbe[+A] extends Product with Serializable {
  ...
  /** Symbols that contains this rbe
    *
    * Example: {{{ symbols(Or(And(Symbol("a",1,3),Symbol("b",1,1)),Symbol("b",2,3))) == Seq("a","b") }}}
    */
  ...
  private def derivSymbol[U >: A](x: U, s: Symbol[U], open: Boolean, controlled: Seq[U])(implicit
      r: Show[U]
  ): Rbe[U] = {
    if (x == s.a) {
      if (s.m == IntLimit(0))
