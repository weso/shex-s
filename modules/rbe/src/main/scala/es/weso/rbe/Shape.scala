...
case class AndShape[Edge, Node, Label, Err, Evidence](
    ...
    s2: Shape[Edge, Node, Label, Err, Evidence]
) extends Shape[Edge, Node, Label, Err, Evidence]
...
case class NotShape[Edge, Node, Label, Err, Evidence](s: Shape[Edge, Node, Label, Err, Evidence])
    extends Shape[Edge, Node, Label, Err, Evidence]

object Shape {

  def empty[Edge, Node, Label, Err, Evidence: Read]: SingleShape[Edge, Node, Label, Err, Evidence] =
    SingleShape(nodeShape = NodeShape.any, rbe = Empty, extras = Seq(), closed = false)

  def singleShape[Edge, Node, Label, Err, Evidence: Read](
      rbe: Rbe[(Edge, NodeShape[Node, Label, Err, Evidence])],
      extras: Seq[Edge] = Seq(),
      closed: Boolean = false
  ): SingleShape[Edge, Node, Label, Err, Evidence] =
    SingleShape(nodeShape = NodeShape.any, rbe = rbe, extras = extras, closed = closed)
