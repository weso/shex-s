...
class DepGraphTest extends FunSuite {
  ...
  test("Should calculate if graph has neg cycles when it hasn't") {
    val g = DepGraph.empty[String].addPosEdge("a", "b").addNegEdge("a", "c").addPosEdge("b", "d").addPosEdge("d", "a")
    assertEquals(g.containsNegCycle, false)
  }
  ...
  test("Should calculate neg cycles") {
    val g = DepGraph.empty[String].addNegEdge("a", "b").addPosEdge("a", "c").addPosEdge("b", "d").addPosEdge("d", "a")
