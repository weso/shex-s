...
case class InheritanceJGraphT[Node, EdgeType](
  ...
  private def collectDescendantsByEdgeType(
      node: Node,
      edgeType: EdgeType,
      visited: Set[Node],
      graph: DirectedAcyclicGraph[Node, Edge[Node, EdgeType]]
    ...
    incomingEdges(node, graph).flatMap(incomingEs => {
      val nodes =
        incomingEs.filter(edge => !visited.contains(edge.sub) && edge.etype == edgeType).map(_.sub)
      nodes
        .map(node => collectDescendantsByEdgeType(node, edgeType, visited ++ nodes, graph))
        .sequence
        .map(_.flatten ++ nodes)
    })
