      ...
      QueryShapeMap(
        List(
          Association(
            node = TriplePattern(
              Focus,
              PredicatePath(rdfType),
              NodePattern(IRI("http://default.org/A"))
            ),
            shape = IRILabel(IRI("http://shapes.org/S"))
          )
        ),
          ...
          .flatMap(
            _.use(rdf =>
              for {
                ...
                shapeMap <- either2IO(
                  Parser.parse(shapeMapStr, None, rdfPrefixMap, shapesPrefixMap)
                )
                expected <- either2IO(
                  Parser.parse(expectedStr, None, rdfPrefixMap, shapesPrefixMap)
                )
