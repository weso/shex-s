...
class ShapeMapTest extends CatsEffectSuite {
  ...
  { // ShapeMaps parser
    ...
    val nodesPrefixMap =
      PrefixMap.empty.addPrefix("", IRI("http://default.org/")).addPrefix("ex", IRI("http://example.org/"))
    val shapesPrefixMap =
      PrefixMap.empty.addPrefix("", IRI("http://default.shapes.org/")).addPrefix("ex", IRI("http://shapes.org/"))
    ...
    shouldParse(
      "23@Start",
      QueryShapeMap(
        List(Association(node = RDFNodeSelector(IntegerLiteral(23)), shape = Start)),
        nodesPrefixMap,
        shapesPrefixMap
      ),
      nodesPrefixMap,
    ...
    shouldParse(
      ":x@Start",
      QueryShapeMap(
        List(Association(node = RDFNodeSelector(IRI("http://default.org/x")), shape = Start)),
        nodesPrefixMap,
        shapesPrefixMap
      ),
      nodesPrefixMap,
    ...
    shouldParse(
      ...
      QueryShapeMap(
        List(
          Association(node = RDFNodeSelector(IRI("http://default.org/x")), shape = IRILabel(IRI("http://shapes.org/S")))
        ),
        nodesPrefixMap,
        shapesPrefixMap
      ),
      ...
      QueryShapeMap(
        List(
          Association(
            ...
            shape = IRILabel(IRI("http://shapes.org/S"))
          )
        ),
        nodesPrefixMap,
        shapesPrefixMap
    ...
    shouldParse(
      """<http://x.com>@<S>""",
      QueryShapeMap(
        List(Association(node = RDFNodeSelector(IRI("http://x.com")), shape = IRILabel(IRI("S")))),
        nodesPrefixMap,
        shapesPrefixMap
      ),
      nodesPrefixMap,
    ...
    def shouldParse(str: String, expected: ShapeMap, nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): Unit = {
  ...
  { // Fix shape map
    ...
    def shouldFixAs(shapeMapStr: String, rdfStr: String, expectedStr: String): Unit = {
      ...
      test(s"should fix $shapeMapStr and obtain $expectedStr") {
        val v: IO[(ShapeMap, ShapeMap)] = RDFAsJenaModel
          .fromString(rdfStr, "TURTLE")
          .flatMap(
            _.use(rdf =>
              for {
          ...
          .flatMap(
            _.use(rdf =>
              for {
                ...
                parsedSm <- fromES(
                  Parser
                    .parse(shapeMapStr, None, rdfPrefixMap, shapesPrefixMap)
                    .leftMap(err => s"Error parsing: ${shapeMapStr}:$err")
                )
                shownSm = parsedSm.toString
                parsedShown <- fromES(
                  Parser
                    .parse(shownSm, None, rdfPrefixMap, shapesPrefixMap)
                    .leftMap(err => s"Error parsing: ${shownSm}:$err")
                )
