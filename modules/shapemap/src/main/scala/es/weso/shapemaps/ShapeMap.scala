...
abstract class ShapeMap {
  ...
  private def showNodeSelector(n: NodeSelector, pm: PrefixMap): String =
    n match {
      ...
      case TriplePattern(sub, path, obj) =>
        s"{${showPattern(sub, pm)} ${showPath(path, pm)} ${showPattern(obj, pm)}}"
  ...
  def showShapeMap(withDetails: Boolean): String = {
    ...
    if (associations.isEmpty)
      ...
      associations
        .map(a => showAssociation(a, nodesPrefixMap, shapesPrefixMap, withDetails))
        .mkString(sep)
  }
  ...
  private def showAssociation(
    ...
    s"${showNodeSelector(a.node, nodesPrefixMap)}@${if (a.info.status == NonConformant) "!"
      else ""}${showShapeMapLabel(a.shape, shapesPrefixMap)}${if (withDetails) showDetails(a.info)
      else ""}"
...
object ShapeMap {
  ...
  def fromURI(
      ...
      nodesPrefixMap: PrefixMap,
      shapesPrefixMap: PrefixMap
  ): IO[Either[NonEmptyList[String], ShapeMap]] = {
    val t = Try {
      ...
      e =>
        IO.raiseError(
          new RuntimeException(
            s"Exception obtaining URI contents. URI = ${uri}. Error: ${e.getLocalizedMessage}"
          )
        ),
      e => IO.pure(e)
    )
  ...
  case class ShapeMapFromPathException(
  ...
  def fromPath(
    ...
    sm <- fromString(str, format, base, nodesPrefixMap, shapesPrefixMap).fold(
      nes =>
        IO.raiseError(
          ShapeMapFromPathException(nes, path, format, base, nodesPrefixMap, shapesPrefixMap)
        ),
      sm => IO.pure(sm)
    )
  } yield sm
  ...
  def parseResultMap(
    ...
    rdf.getPrefixMap.flatMap(rdfPrefixMap =>
      IO(
        Parser
          .parse(str, base, rdfPrefixMap, shapesPrefixMap)
          .fold(
            e => throw new RuntimeException(s"Error parsing as ShapeMap str:$str\nError: $e"),
            identity
          )
      ).flatMap(queryMap =>
  ...
  def fixShapeMap(
    ...
    val empty: IO[FixedShapeMap] = IO.pure(
      FixedShapeMap.empty.addNodesPrefixMap(nodesPrefixMap).addShapesPrefixMap(shapesPrefixMap)
    )
    ...
    def addNode(a: Association)(
      ...
      newShapeMap <- IO {
        fixed
          .addAssociation(Association(RDFNodeSelector(node), a.shape, a.info))
          .fold(
            e => throw new RuntimeException(s"Error adding association: $a to $fixed"),
            identity
          )
      }
    } yield newShapeMap

    def combine(a: Association, current: IO[FixedShapeMap]): IO[FixedShapeMap] =
      for {
