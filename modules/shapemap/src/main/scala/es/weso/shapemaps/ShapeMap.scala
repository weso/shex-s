...
abstract class ShapeMap {
  ...
  private def showAssociation(
    ...
    s"${showNodeSelector(a.node, nodesPrefixMap)}@${if (a.info.status == NonConformant) "!" else ""}${showShapeMapLabel(
      a.shape,
      shapesPrefixMap
    )}${if (withDetails) showDetails(a.info) else ""}"
  }
...
object ShapeMap {
  ...
  def fromURI(
    ...
    t.fold(
      e =>
        IO.raiseError(
          new RuntimeException(s"Exception obtaining URI contents. URI = ${uri}. Error: ${e.getLocalizedMessage}")
        ),
      e => IO.pure(e)
    )
  }
    ...
    for {
      ...
      queryMap <- IO {
        Parser
          ...
          .fold(e => throw new RuntimeException(s"Error parsing as ShapeMap str:$str\nError: $e"), identity)
      }
