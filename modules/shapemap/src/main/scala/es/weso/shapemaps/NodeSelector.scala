...
case class TriplePattern(subjectPattern: Pattern, path: SHACLPath, objectPattern: Pattern) extends NodeSelector {
  ...
  override def relativize(base: IRI): NodeSelector =
    TriplePattern(subjectPattern.relativize(base), path.relativize(base), objectPattern.relativize(base))
    ...
    final def apply(nodeSelector: NodeSelector): Json = {
      nodeSelector match {
        ...
        case TriplePattern(subj, path, obj) => {
          Json.fromJsonObject(
            JsonObject.empty.add("subject", subj.asJson).add("path", path.asJson).add("object", obj.asJson)
          )
        }
