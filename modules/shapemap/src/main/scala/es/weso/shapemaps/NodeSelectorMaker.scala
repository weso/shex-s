...
/** Visits the AST and builds the corresponding ShapeMaps classes
  */
class NodeSelectorMaker(base: Option[String], nodesPrefixMap: PrefixMap)
    extends NodeSelectorBaseVisitor[Any] {
  ...
  override def visitPathAlternative(ctx: PathAlternativeContext): Builder[SHACLPath] =
    for {
      alts <- {
        val r: List[Builder[SHACLPath]] =
          ctx.pathSequence().asScala.map(visitPathSequence(_)).toList
  ...
  override def visitPathSequence(ctx: PathSequenceContext): Builder[SHACLPath] =
    for {
      seqs <- {
        val r: List[Builder[SHACLPath]] =
          ctx.pathEltOrInverse().asScala.map(visitPathEltOrInverse(_)).toList
        r.sequence[Builder, SHACLPath]
      }
    } yield
  ...
  override def visitPathPrimary(ctx: PathPrimaryContext): Builder[SHACLPath] = ctx match {
    case _ if isDefined(ctx.nodeIri()) =>
      visitNodeIri(ctx.nodeIri(), nodesPrefixMap).map(PredicatePath(_))
