...
case class ResultShapeMap(
  ...
  def getInfoAll: List[(ShapeMapLabel, Info)] =
    resultMap.values.foldLeft(List.empty[(ShapeMapLabel, Info)]) { (acc, nodeResult) =>
      acc ++ nodeResult
    }

  // Quick way to find if all results are conformant (and thus if the whole thing
  // is valid)
  def isAllConformant: Boolean = getInfoAll.forall { case (_, info) =>
  ...
  private def compareShapes(
    ...
    if (shapes1.keySet.count(!_.isBNodeLabel) != shapes2.keySet.count(!_.isBNodeLabel))
      ...
      val es: List[Either[String, Boolean]] = shapes1
        .filter(!_._1.isBNodeLabel)
        .map { case (label, info1) =>
          shapes2.get(label) match {
            case None =>
              Left(
                s"Node $node has label $label in map1 but doesn't have that label in map2. Shapes1 = $shapes1, Shapes2 = $shapes2"
              )
            case Some(info2) =>
