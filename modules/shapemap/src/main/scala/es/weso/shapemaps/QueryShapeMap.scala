...
/** Input shape map also known as Query shape map
  * @param associations
  */
case class QueryShapeMap(associations: List[Association], nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap)
    extends ShapeMap {
  ...
  override def relativize(maybeBase: Option[IRI]): QueryShapeMap = maybeBase match {
    case None       => this
    case Some(base) => QueryShapeMap(associations.map(_.relativize(base)), nodesPrefixMap, shapesPrefixMap)
  }
}
