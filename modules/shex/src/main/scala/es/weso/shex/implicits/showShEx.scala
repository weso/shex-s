...
object showShEx {
  ...
  implicit lazy val showShape: Show[Shape] = new Show[Shape] {
    final def show(a: Shape): String = a match {
      ...
      case _ =>
        s"${optShow(a.id)}${optShowBoolean(a.virtual, "VIRTUAL")}${optShowBoolean(a.closed, " CLOSED")}${optShowExtras(
          a.extra
        )}${optShowExtends(a._extends)}${optShowRestricts(a.restricts)}{ ${optShow(a.expression)} ${optShowLs(a.actions, "\n")} }"
    }
  }
