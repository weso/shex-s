...
case class ValidatorRef(
    ...
    rdf.getPrefixMap.flatMap(prefixMap =>
      ShapeMap
        ...
        .fold(
          ...
          sm =>
            ShapeMap.fixShapeMap(sm, rdf, prefixMap, schema.prefixMap).flatMap(fm => validateShapeMap(rdf, fm, verbose))
        )
    )
  }
  ...
  private def morePending(refState: Ref[IO, State]): IO[Boolean] =
    getState(refState).flatMap(state => state.pending.isEmpty.pure[IO])

  private def evaluatePending(refState: Ref[IO, State]): IO[Unit] =
    getState(refState).flatMap(state => state.pending.parFoldMapA(validateNodeShapePending(refState)))

  private def validateNodeShapePending(refState: Ref[IO, State])(node: RDFNode, shapeLabel: ShapeMapLabel): IO[Unit] =
    refState
      .updateAndGet(_.changePending(node, shapeLabel))
      .flatMap(newState =>
        newState.shapeMap
          .get(node)
          .fold(
  ...
  private def getResult(refState: Ref[IO, State]): IO[Result] =
    getState(refState).flatMap(state => {
      ...
      val rsm: ResultShapeMap =
        ResultShapeMap(state.shapeMap, nodesPrefixMap = state.nodesPrefixMap, shapesPrefixMap = state.shapesPrefixMap)
