...
case class DatatypeEvidence(node: RDFNode, datatype: IRI) extends NodeConstraintEvidence(s"${node} has datatype ${datatype}")
case class ValueSet(node: RDFNode, conforms: List[ValueSetValue], valueSet: List[ValueSetValue]) extends NodeConstraintEvidence(s"${node} conforms to values [${conforms.map(_.toString).mkString(",")}] for valueSet: ${valueSet.map(_.toString).mkString(",")}")
