...
object ShExError {
  ...
  case class NotEnoughArcs(node: RDFNode, values: Set[RDFNode], path: Path, min: Int, rdf: RDFReader)
  ...
  case class ErrCardinality(attempt: Attempt, node: RDFNode, path: Path, values: Int, card: Cardinality, rdf: RDFReader)
      extends ShExError(s"Cardinality error. Node: $node. Cardinality: ${card.show}") {
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""${attempt.showQualified(nodesPrefixMap, shapesPrefixMap)}: # of values for ${path.showQualified(
        shapesPrefixMap
      )}=$values doesn't match ${card.show}"""
    }
  ...
  case class CheckDatatypeError(node: RDFNode, datatype: IRI, rdf: RDFReader)
    ...
    override def toJson: Json = {
      ...
      Json.fromFields(List(("type", Json.fromString("CheckDatatypeError")), ("node", node2Json(node, rdf))))
    }
  }
  ...
  case class ExtraPropertiesClosedShape(node: RDFNode, ps: Set[IRI], shape: Shape, rdf: RDFReader)
    ...
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""|Closed shape ${shape.showQualified(shapesPrefixMap)} with extra properties at node: ${nodesPrefixMap.qualify(
        node
      )})
  ...
  case class SemanticActionException(attempt: Attempt, node: RDFNode, action: SemAct, exc: Throwable)
  ...
  case class AbstractShapeErr(node: RDFNode, shape: ShapeLabel, rdf: RDFReader)
    ...
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""AbstractShapeError ${nodesPrefixMap.qualify(node)} cannot conform to abstract shape ${shapesPrefixMap.qualify(
        shape.toRDFNode
      )}"""
    }
  ...
  case class ExtendFails(
      ...
      err: ShExError,
      rdf: RDFReader
  ) extends ShExError(s"""|ExtendFails: ${node.show} doesn't conform to extended shape ${extended.toRDFNode.show}
          |  Error obtained: ${err.msg}""".stripMargin) {
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""|Node ${nodesPrefixMap.qualify(node)} doesn't conform to extended shape ${shapesPrefixMap.qualify(
        extended.toRDFNode
      )}
            |Attempt: ${attempt.showQualified(nodesPrefixMap, shapesPrefixMap)}
            |Error: ${err.showQualified(nodesPrefixMap, shapesPrefixMap)}
  ...
  case class NoPartition(
      ...
      schema: AbstractSchema
        ...
        s"""|No partition of neighs from node ${node.show} matches shape ${s.id.map(_.toRDFNode.show).getOrElse("")}. 
                            |Neighs = ${neighs} 
...
//       ("neighs", neighs.asJson),
  ...
  case class PartitionFailed(
      ...
      pair: (Set[Arc], Set[Arc])
        ...
        s"""|Partition of neighs from node ${node.show} failed to match ${s.id.map(_.toRDFNode.show).getOrElse("")}. 
                            |Partition = ${pair} 
                            |Extend label: ${extendLabel.toRDFNode.show}""".stripMargin
      ) {
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""|Partition of neighs from node ${nodesPrefixMap.qualify(node)} failed to match shape ${shapesPrefixMap
        .qualify(extendLabel.toRDFNode)}
  ...
  case class ClosedShapeWithRests(
      ...
      extras: List[Path]
        ...
        .mkString(",")} or ${extras.map(_.show).mkString(",")}
                          |""".stripMargin) {
    override def showQualified(nodesPrefixMap: PrefixMap, shapesPrefixMap: PrefixMap): String = {
      s"""|Closed shape but rest: ${rest.path.showQualified(shapesPrefixMap)} is not in ${ignoredPathsClosed
        .map(_.show)
        .mkString(",")} or ${extras.map(_.show).mkString(",")}
          |""".stripMargin
    }
