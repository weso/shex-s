...
object ValueChecker {
  ...
  /** Checks if a node matches a valueSet value
    ...
    */
  def valueChecker(node: RDFNode, value: ValueSetValue): Either[String, String] =
    value match {
      case IRIValue(iri) =>
        node match {
          case i: IRI =>
            if (iri == i) Right(s"${iri.show} is equal to ${i.show}")
      ...
      case StringValue(s) =>
        node match {
          case l: Literal =>
            if (s == l.getLexicalForm && l.dataType == `xsd:string`)
              Right(s"${node.show} == ${l}")
            else
              Left(s"${node.show} != ${l}")
      ...
      case LanguageStem(stem) =>
        node match {
          case l: LangLiteral =>
            if (checkLanguageStem(l.lang, stem))
              Right(s"${node.show} lang(${l.lang}) matches ${stem}")
      ...
      case is @ IRIStemRange(stem, excls) =>
        node match {
          case i: IRI =>
            if (checkIRIStemRange(i, stem, excls))
      ...
      case ls @ LiteralStemRange(stem, excls) =>
        node match {
          case l: Literal =>
            if (checkLiteralStemRange(l, stem, excls))
              Right(s"${node.show} matches Literal stem range $ls")
  ...
  private def checkLanguageStem(l: Lang, stem: Lang): Boolean = {
    ...
    if (strStem.isEmpty) true
    else {
      strL === strStem || strL.startsWith(strStem + "-")
    }
  }

  private def checkIRIStemRange(
      iri: IRI,
      stemRange: IRIStemRangeValue,
      excls: Option[List[IRIExclusion]]
  ): Boolean = {
  ...
  private def checkLiteralStemRange(
      lit: Literal,
      stemRange: LiteralStemRangeValue,
      excls: Option[List[LiteralExclusion]]
  ): Boolean = {
    val cond1 = stemRange match {
  ...
  private def checkLangStemRange(
      lang: Lang,
      stemRange: LanguageStemRangeValue,
      excls: Option[List[LanguageExclusion]]
  ): Boolean = {
    val cond1 = stemRange match {
