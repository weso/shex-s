...
trait AvailableShapeExprPaths extends ExtendM {
   ...
   */
    ...
    def finder(lbl: ShapeLabel): IO[ShapeExpr] = { 
     schema
      .getShape(lbl)
      .leftMap(ShExError.LabelNotFound(lbl, _, schema.labels)).fold(
    ...
    def extend(se: ShapeExpr): IO[List[ShapeLabel]] = se match {
      case shape: Shape => IO.pure(shape._extends.getOrElse(List()))
      case sd: ShapeDecl => extend(sd.shapeExpr)
      case _ => IO(List())
    }
    
    def first(se: ShapeExpr): IO[List[(ShapeExpr, Available[Path])]] = 
     rest(se).flatMap(rs => parent match {
      case None => rs.pure
      case Some(lbl) => rs.pure 
     })

    def getSEsLabels(ds: List[ShapeLabel]): IO[List[ShapeExpr]] = ds.map(finder(_)).sequence
    def getSEPathsSEs(ses: List[ShapeExpr]): IO[List[(ShapeExpr, Available[Path])]] = ses.map(rest(_)).sequence.map(_.flatten)
      
    def rest(se: ShapeExpr): IO[List[(ShapeExpr, Available[Path])]] = 
      getAvailablePathsSE(se).map(as => List((se,as)))
    
    def getAvailablePathsSE(se: ShapeExpr): IO[Available[Path]] = 
      se.paths(schema).fold(
        err => IO.raiseError(StringError(err)), 
        Available(_).pure
      )

    extendCheckingVisitedM1[ShapeExpr, List[(ShapeExpr, Available[Path])], ShapeLabel,IO](s, finder, extend, first, rest)
