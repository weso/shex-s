  ...
  private def mkTableAux(
    ...
    te match {
      ...
      case e: EachOf =>
        ...
        def comb(
            rest: Either[String, ResultPair],
            currentTe: TripleExpr
        ): Either[String, ResultPair] = for {
          pair1 <- rest
      ...
      case Inclusion(label) =>
        teMap.get(label) match {
  ...
  implicit lazy val showCTable: Show[CTable] = new Show[CTable] {
    override def show(table: CTable) = {
      ...
      def showPaths(paths: PathsMap): String =
        paths
          .map { case (path, cr) =>
            s"${path.show}->${cr.map(_.show).mkString(",")}"
          }
          .mkString(s"\n")
