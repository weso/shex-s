...
//  val builder: RDFBuilder
  ...
  def run[A](c: Check[A])(config: Config)(env: Env): IO[(Log, Either[Err, A])] = {
    c.value
      .run(ConfigEnv(config, env), State(ShapeTyping.emptyShapeTyping))
      .map(vs => {
        val (log, env, c) = vs
        (log, c)
      })
  ...
  def checkCond(condition: Boolean, attempt: Attempt, error: ShExError, evidence: String): CheckTyping = for {
      ...
      case None =>
        getRDF.flatMap(rdf =>
          outgoingTriples(node, rdf).flatMap(outgoing =>
            incomingTriples(node, rdf).flatMap(incoming => ok(Neighs.fromList(outgoing ++ incoming)))
          )
        )
      case Some(ns) => ok(ns)
    }
