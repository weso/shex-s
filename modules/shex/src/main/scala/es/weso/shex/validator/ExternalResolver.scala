...
sealed abstract class ExternalResolver(name: String) {
  ...
  def getShapeExpr(label: ShapeLabel, as: Option[List[Annotation]]): IO[ShapeExpr]
}
...
object ExternalResolver {
  ...
  case class ExternalIRIResolver(iri: IRI, verbose: VerboseLevel) extends ExternalResolver("ExternalIRIResolver") {
    ...
    override def getShapeExpr(label: ShapeLabel, as: Option[List[Annotation]]): IO[ShapeExpr] = for {
      schema <- ioSchema
