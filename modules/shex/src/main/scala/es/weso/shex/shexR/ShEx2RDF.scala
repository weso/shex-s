...
trait ShEx2RDF extends RDFSaver {
  ...
  private def valueSetValue(x: ValueSetValue): RDFSaver[RDFNode] = x match {
    ...
    case Language(Lang(str)) =>
      ok(StringLiteral(str)) // TODO: This one looks suspicious (it seems to have the same representation as a string)
  ...
  private def tripleExpr(te: TripleExpr): RDFSaver[RDFNode] =
    te match {
      ...
      case TripleConstraint(id, inverse, negated, pred, valueExpr, min, max, _, semActs, annotations) =>
        for {
          teId <- mkId(id)
