...
object decoderShEx {
  ...
  // TODO: Move to Utils
  private def optFixedFieldValue(c: HCursor, name: String, value: String): Decoder.Result[Unit] =
    for {
      ...
      r <- maybeValue match {
        ...
        case Some(v) =>
          if (v == value) Either.right(())
            ...
            Either.left(
              DecodingFailure(s"$value for field $name should be \n$value but got\n$v", Nil)
            )
      }
    } yield r
  ...
  implicit lazy val decodeLanguageRangeStemWildcard: Decoder[LanguageStemRangeWildcard] =
    Decoder.instance { c =>
      for {
        _ <- fixedFieldValue(c, "type", "Wildcard")
  ...
  implicit lazy val decodeLiteralStemRangeWildcard: Decoder[LiteralStemRangeWildcard] =
    Decoder.instance { c =>
      for {
        _ <- fixedFieldValue(c, "type", "Wildcard")
