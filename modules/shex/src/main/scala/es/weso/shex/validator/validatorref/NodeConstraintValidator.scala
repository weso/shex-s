...
object NodeConstraintValidator {
    ...
    private def nodeKind(nk: NodeKind, node: RDFNode): ValidatedNel[NodeConstraintError, NodeConstraintEvidence] = nk match {
      case IRIKind => if (node.isIRI) IRIKindEvidence(node).valid
                      else IRIKindError(node).invalidNel
      case BNodeKind => if (node.isBNode) BNodeKindEvidence(node).valid
                        else BNodeKindError(node).invalidNel
      case NonLiteralKind => if (!node.isLiteral) NonLiteralKindEvidence(node).valid
                             else NonLiteralKindError(node).invalidNel
      case LiteralKind => if (node.isLiteral) LiteralKindEvidence(node).valid
    ...
    private def datatype(dt: IRI, node: RDFNode): ValidatedNel[NodeConstraintError, NodeConstraintEvidence] = 
      node match {
          case literal: Literal => if (literal.dataType == dt) DatatypeEvidence(node, dt).valid
