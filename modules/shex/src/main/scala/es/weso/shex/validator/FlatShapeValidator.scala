              ...
              val p: Check[ShapeTyping] = for {
                ...
                t <-
                  if (card.contains(passed.size)) {
                    addEvidence(
                      attempt.nodeShape,
                      s"Number of values for ${showNode(node)} with ${path.showQualified(nodesPrefixMap)} that satisfy ${constraint.shape
                          .map(showSE(_))
                  ...
                  fromIO(partitionEitherIOS(rs)).flatMap(partition => {
                    ...
                    if (notPassed.isEmpty) {
                      debug(s"notPassed is empty => all passed :)") *>
                        addEvidence(
                          attempt.nodeShape,
                          s"${showNode(node)} passed ${constraint.showQualified(shapesPrefixMap)} for path ${path.showQualified(nodesPrefixMap)}"
                        )
                    } else {
