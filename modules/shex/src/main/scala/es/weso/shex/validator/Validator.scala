  ...
  private[validator] def checkNodeLabel(
    ...
    getTyping.flatMap(typing =>
      getNodesPrefixMap.flatMap(nodesPrefixMap =>
        infoTyping(
          typing,
          s"checkNodeLabel(${node.show},${label.show}). Visited: ${showLs(visited)}, ext: ${showExt(ext)}",
          nodesPrefixMap
        ) *>
          (if (typing.hasInfoAbout(node, label))
             debug(
               s"checkNodeLabel(${node.show},${label.show}). Typing already contains label ${label.show}. Visited: ${visited}, Ext: ${showExt(ext)}"
             ) *>
               ok(typing)
    ...
    getTyping.flatMap(typing =>
      getNodesPrefixMap.flatMap(nodesPrefixMap =>
        infoTyping(
          ...
          checkNodeLabel(node, ref, ext, Set()).flatMap(t => checkHasType(node, t, attempt)(ref).flatMap(_ => ok(t)))
      )
    )
