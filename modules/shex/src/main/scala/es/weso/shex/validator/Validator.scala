...
case class Validator(
  ...
  private def checkTargetNodeDeclarations: CheckTyping =
    for {
      ...
      ts <- checkAll(nodeLabels.map { case (node, label) =>
        checkNodeLabel(node, label, None, Visited.empty)
      })
      t <- combineTypings(ts)
    } yield t
  ...
  private def checkNodeShapeLabel(node: RDFNode, shape: ShapeLabel): CheckTyping = {
    cond(
      ...
      (shapeLabel: ShapeLabel) => checkNodeLabel(node, shapeLabel, None, Visited.empty),
      err =>
        getTyping
  ...
  private def checkNodeStart(node: RDFNode): CheckTyping = getRDF.flatMap { rdf =>
    schema.start match {
      ...
      case Some(shape) => {
        ...
        runLocalSafeTyping(
          satisfies(node, shape, None, Visited.empty, attempt),
  ...
  private[validator] def checkNodeLabel(
    ...
    getTyping.flatMap(typing =>
      getNodesPrefixMap.flatMap(nodesPrefixMap =>
        infoTyping(
          typing,
          s"checkNodeLabel(${node.show},${label.show}). ${visited.show(schema)}, ext: ${showExt(ext)}",
          nodesPrefixMap
        ) *>
      ...
      (s match {
        ...
        case _ if s.isFlatShape(schema) =>
          for {
            ...
            typing <-
              FlatShapeValidator(this, pm, schema.prefixMap, builder, schema).checkFlatShape(
                attempt,
                node,
