...
case class Validator(
  ...
  private def checkTargetNodeDeclarations: CheckTyping =
    for {
      ...
      ts <- checkAll(nodeLabels.map { case (node, label) =>
        checkNodeLabel(node, label, None, Set())
      })
      t <- combineTypings(ts)
    } yield t
  ...
  private def checkNodeStart(node: RDFNode): CheckTyping = getRDF.flatMap { rdf =>
    schema.start match {
      ...
      case Some(shape) => {
        ...
        runLocalSafeTyping(
          satisfies(node, shape, None, Set(), attempt),
  ...
  private[validator] def checkNodeLabel(
    ...
    getTyping.flatMap(typing =>
      getNodesPrefixMap.flatMap(nodesPrefixMap =>
        infoTyping(
          typing,
          s"checkNodeLabel(${node.show},${label.show}). ${showVisited(visited)}, ext: ${showExt(ext)}",
          nodesPrefixMap
        ) *>
          (if (typing.hasInfoAbout(node, label))
             debug(
               s"checkNodeLabel(${node.show},${label.show}). Typing already contains label ${label.show}. ${showVisited(visited)}, Ext: ${showExt(ext)}"
             ) *>
               ok(typing)
           else if (visited.contains(label)) {
             debug(
               s"checkNodeLabel(${node.show},${label.show}). Visited already contains label ${label.show}. Visited: ${visited} Ext: ${showExt(ext)}"
             ) *>
               ok(typing)
           } else {
             cond(
               getShape(label),
               (se: ShapeExpr) => checkNodeLabelSafe(node, label, se, ext, visited + label),
  ...
  private def checkDescendants(
    ...
    getDescendants(se).flatMap(descendants => {
      ...
      if (nonAbstractDs.isEmpty) err(e)
        ...
        debug(
          s"Checking descendants(${node.show}@${showSE(se)}) ${nonAbstractDs.map(schema.qualify(_)).mkString(",")}, ${showVisited(visited)}"
        ) *>
          checkSome(cs, ShapeExprFailedAndNoDescendants(attempt, node, se, e, nonAbstractDs, schema)).flatMap(t1 =>
            addEvidence(attempt.nodeShape, s"${node.show} passes one descendant").flatMap(t2 => combineTypings(t1, t2))
          )
      }
    })
  ...
  private def checkAnd(
    ...
    debug(s"And($node, ${ses.map(showSE(_)).mkString(",")})") *>
      checkAll(ses.map(se => satisfies(node, se, ext, visited, attempt))).flatMap(ts => combineTypings(ts))
    ...
    debug(s"ShapeDecl(${node.show}, ${showSE(sd)})") *>
      satisfies(node, sd.shapeExpr, ext, visited, attempt).flatMap(t =>
        getDescendants(sd).flatMap(descendants => {
          ...
          debug(
            s"ShapeDecl(${node.show}, ${showSE(sd)})1: nonAbstractDs = [${nonAbstractDs.map(schema.qualify(_)).mkString(",")}]"
          ) *>
  ...
  private def checkRef(
    ...
    getTyping.flatMap(typing =>
      getNodesPrefixMap.flatMap(nodesPrefixMap =>
        infoTyping(
          typing,
          s"Ref: ${node.show}@${schema.qualify(ref)}, ext: ${showExt(ext)}, ${showVisited(visited)}",
          nodesPrefixMap
        ) *>
          checkNodeLabel(node, ref, ext, visited).flatMap(t => checkHasType(node, t, attempt)(ref).flatMap(_ => ok(t)))
      )
    )
  ...
  private def checkShapeBase(
    ...
    debug(s"checkShapeBase(${node.show}@{${showSE(s)}}, flatShape? ${s
      .isFlatShape(schema)}, visited: ${showLs(visited)}, ext: ${showExt(ext)}") *>
      (s match {
        ...
        case _ if s.isFlatShape(schema) =>
          for {
            ...
            typing <- FlatShapeValidator(this, pm, schema.prefixMap, builder, schema).checkFlatShape(
              attempt,
              node,
        ...
        bag => {
          ...
          val checkNodeConstraints: List[CheckTyping] = nodeConstraints.map {
            case (node, pair) => {
              ...
              satisfies(node, shapeExpr, None, visited, attempt)
