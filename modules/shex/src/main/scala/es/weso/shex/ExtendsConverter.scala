...
object ExtendsConverter {
  ...
  private def transShape(schema: ResolvedSchema, shape: Shape): Cnv[Shape] =
    shape._extends match {
      ...
      case Some(es) =>
        info(
          s"transShape (shape=${shape.showQualified(schema.prefixMap)}) with extends: ${es.map(schema.qualify(_)).mkString(",")}\n"
        ) *>
  ...
  private def embedLs(schema: ResolvedSchema)(ls: List[ShapeLabel], shape: Shape): Cnv[Shape] = {
    def cmb(s: Shape, lbl: ShapeLabel): Cnv[Shape] = for {
      ...
      _ <- info(
        s"embedLs.comb(s=${s.showQualified(schema.prefixMap)}, lbl=${schema.qualify(lbl)})=${newSe.showQualified(schema.prefixMap)}"
      )
    } yield newSe
  ...
  private def embedShape(schema: ResolvedSchema)(shape1: Shape, shape2: Shape): Cnv[Shape] =
    for {
      _ <- info(
        s"before embedShape(shape1=${shape1.showQualified(schema.prefixMap)},shape2=${shape2.showQualified(schema.prefixMap)})"
      )
  ...
  def getShapeLabel(schema: ResolvedSchema)(sl: ShapeLabel): Cnv[ShapeExpr] =
    schema.getShape(sl) match {
      case Left(str) =>
        err(
          s"Not found label: ${sl.toRDFNode.getLexicalForm} in schema. Available labels: ${schema.labels.map(_.toRDFNode.getLexicalForm).mkString(",")}"
        )
      case Right(se) => ok(se)
    }
