...
object ExtendsConverter {
  ...
  private def withVisited[A](lbl: ShapeLabel, cnv: Cnv[A]): Cnv[A] = ReaderT.local((ls: Set[ShapeLabel]) => ls + lbl)(cnv)
  ...
  private def cnvMap(s: ResolvedSchema)(m: Map[ShapeLabel, ResolvedShapeExpr]): Cnv[Map[ShapeLabel,ResolvedShapeExpr]] = 
  ...
  private def transShape(schema: ResolvedSchema, shape: Shape): Cnv[Shape] = 
   shape._extends match {
    case None | Some(Nil) => shape.expression match {
     case Some(te) => for {
    ...
    case Some(es) => 
      info(s"transShape (shape=${shape.showQualified(schema.prefixMap)}) with extends: ${es.map(schema.qualify(_)).mkString(",")}\n") *>
  ...
  private def embedLs(schema: ResolvedSchema)(ls: List[ShapeLabel], shape: Shape): Cnv[Shape] = {
    def cmb(s: Shape, lbl: ShapeLabel): Cnv[Shape] = for {
      _ <- info(s"embedLs.comb(${s.showQualified(schema.prefixMap)}, lbl=${schema.qualify(lbl)})???")
...
/*      tse <- s.id match {
      ...
      _ <- info(s"embedLs.comb(s=${s.showQualified(schema.prefixMap)}, lbl=${schema.qualify(lbl)})=${newSe.showQualified(schema.prefixMap)}")
    } yield newSe
    Foldable[List].foldM(ls,shape)(cmb)
  }

  private def embedSe(schema: ResolvedSchema)(se: ShapeExpr, shape: Shape): Cnv[Shape] = 
   info(s"embedSe(${se.showQualified(schema.prefixMap)},${shape.showQualified(schema.prefixMap)})") *>
   (se match {
    case nc: NodeConstraint => err(s"embed(NodeConstraint($nc),${shape}")
    case s: Shape => for {
      r <- embedShape(schema)(s,shape)
      _ <- info(s"embedSe(se=${se.showQualified(schema.prefixMap)},shape=${shape.showQualified(schema.prefixMap)})=${r.showQualified(schema.prefixMap)}") 
    } yield r
    case sa: ShapeAnd => /* {
  ...
  private def embedShape(schema:ResolvedSchema)(shape1:Shape, shape2:Shape): Cnv[Shape] = 
    for {
     _ <- info(s"before embedShape(shape1=${shape1.showQualified(schema.prefixMap)},shape2=${shape2.showQualified(schema.prefixMap)})")
     ...
     _ <- info(s"after embedShape(shape1=${shape1.showQualified(schema.prefixMap)},shape2=${shape2.showQualified(schema.prefixMap)})=${r.showQualified(schema.prefixMap)}") 
    } yield r
    
  private def embedOptTe(schema: ResolvedSchema)(maybeTe1: Option[TripleExpr], maybeTe2: Option[TripleExpr]): Option[TripleExpr] = 
  ...
  def getShapeLabel(schema: ResolvedSchema)(sl: ShapeLabel): Cnv[ShapeExpr] = 
    schema.getShape(sl) match {
      case Left(str) => err(s"Not found label: ${sl.toRDFNode.getLexicalForm} in schema. Available labels: ${schema.labels.map(_.toRDFNode.getLexicalForm).mkString(",")}")
      case Right(se) => ok(se)
    }
