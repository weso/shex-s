...
class NodeConstraintValidatorTest extends CatsEffectSuite {
 ...
 test("IRIKind ok") {
  ...
  NodeConstraintValidator.validateNodeConstraint(node,nc)
 ...
 test("ValueSet 2 in [1 2 3] ") {
  val nc = NodeConstraint.valueSet(List(ObjectValue.intValue(1), ObjectValue.intValue(2), ObjectValue.intValue(3)), List())
