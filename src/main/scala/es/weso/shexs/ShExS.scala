    ...
    val cmp: IO[ResultShapeMap] = for {
      ...
      v <- (res1, res2).tupled.use { case (rdf, builder) =>
        for {
          ...
          shapeMap <- fromES(
            ShapeMap
              .fromInputStream(shapeMap, options.shapemapFormat, options.base, prefixMap, resolved.prefixMap)
    ...
    val cmp: IO[ResultShapeMap] = for {
      ...
      v <- (res1, res2).tupled.use { case (rdf, builder) =>
        for {
          ...
          shapeMap <- fromES(
            ShapeMap
              .fromString(shapeMapStr, options.shapemapFormat, options.base, prefixMap, resolved.prefixMap)
