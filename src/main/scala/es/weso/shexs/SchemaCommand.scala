...
object ShowMethod {
  ...
  val showMethodOpt: Opts[ShowMethod] =
    Opts
      ...
      .mapValidated(s =>
        showMethods.collect { case sm if sm.name.toLowerCase == s.toLowerCase => sm }.headOption match {
          case None =>
            invalidNel(s"Invalid show method $s. Available methods = ${showMethods.map(_.name).mkString(",")}")
