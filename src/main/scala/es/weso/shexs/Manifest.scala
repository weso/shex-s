...
case class Manifest(
  ...
  def run(): IO[ExitCode] = {
    val assumeLocal: Option[(IRI, Path)] = Some(
      (
        IRI("https://raw.githubusercontent.com/shexSpec/shexTest/master/"),
        Paths.get("src/test/resources/shexTest")
      )
    )
    parseManifest(
      manifestFileName,
      ...
      testName,
      List(),
      validatorVersion.buildValidator,
      timeout,
      assumeLocal,
      verbose
    ).flatMap(results =>
  ...
  lazy val testName: Opts[TestSelector] =
    Opts
      .option[String](
        "test-selector",
        short = "n",
        help = "Test selector (if none provided, it will run all"
      )
      .orNone
