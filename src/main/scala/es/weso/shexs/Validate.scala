...
case class Validate(
  ...
  def run(): IO[ExitCode] = for {
    ...
    vv <- (res1, res2).tupled.use { case (rdf, builder) =>
      for {
        ...
        validator = validatorVersion.buildValidator(
          resolvedSchema,
  ...
  private def getRDFData(dataSpec: DataSpec, baseIRI: Option[IRI]): IO[Resource[IO, RDFReader]] =
    dataSpec match {
      case DataPath(dataPath, dataFormat) =>
        RDFAsJenaModel.fromURI(
          dataPath.toUri().toString(),
          dataFormat.getOrElse(DataFormat.defaultDataFormat),
          baseIRI
        )
