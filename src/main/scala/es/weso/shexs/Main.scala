...
object Main
  ...
  lazy val mappingOpt =
    Opts.option[Path]("mapping", short = "m", metavar = "mappings-file", help = "Path to Mappings file.")
  ...
  lazy val dataPath: Opts[DataPath] = (dataOpt, dataFormatOpt).mapN { case (path, format) =>
    DataPath(path, Some(format))
  }
  ...
  def getRDFData(dataSpec: DataSpec, baseIRI: Option[IRI]): IO[Resource[IO, RDFReader]] = dataSpec match {
    case DataPath(dataPath, dataFormat) =>
      RDFAsJenaModel.fromURI(dataPath.toUri().toString(), dataFormat.getOrElse(defaultDataFormat), baseIRI)
    ...
    for {
      ...
      vv <- (res1, res2).tupled.use { case (rdf, builder) =>
        for {
          ...
          shapeMap <- getShapeMapFromFile(
            vc.shapeMapSpec.shapeMap,
            vc.shapeMapSpec.shapeMapFormat,
            nodesPrefixMap,
            schema.prefixMap,
            vc.schemaSpec.baseIRI
          )
      ...
      for {
        ...
        vv <- (res1, res2).tupled.use { case (rdf, builder) =>
          for {
            ...
            shapeMap <- getShapeMapFromFile(
              wc.shapeMapSpec.shapeMap,
              wc.shapeMapSpec.shapeMapFormat,
              nodesPrefixMap,
              schema.prefixMap,
              wc.schemaSpec.baseIRI
            )
            fixedMap       <- ShapeMap.fixShapeMap(shapeMap, rdf, nodesPrefixMap, resolvedSchema.prefixMap)
