...
object WikibaseValidate {
 ...
 lazy val wikibaseCommand: Opts[WikibaseValidate] = 
    Opts.subcommand("wikibase", "Validate RDF data from wikibase") {
      (SchemaSpec.schemaSpec, EndpointOpt.endpoint, prefixMapPath, ShapeMapSpec.shapeMapSpec, ShowResult.showResultFormatOpt, OutputOpt.outputOpt, VerboseLevelOpt.verboseLevel)
